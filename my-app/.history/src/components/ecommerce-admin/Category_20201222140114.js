...
const Category = ({ category, callbackDeletedCategory }) => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:9000/products?id_category=${category.id}`)
            .then(res => {
                ...
                console.log('cand luam produsele pt o categ',select)
    ...
    function getEditData(updatedProduct) {
        ...
        for (let op of select) {
            if (op.id == updatedProduct.id) {
                indexOfOld = select.indexOf(op);
                break;
            }
        }
        let copyOldProducts = [...select];
    ...
    let productsComponents = select.map(prodObject => <Product getEditData={getEditData} products={select} onClick={handleCheckbox} product={prodObject} key={prodObject.id} />);

    return (
        <div className="category">
            {!showBtns && <button onClick={showModalEdit}>Edit category</button>}
            <ModalEditCategory getEditData={getEditData} initialProducts={select} showEditModalFlag={showEditModalFlag} hideModalEdit={hideModalEdit} />
