...
const Category = ({ category, callbackDeletedCategory }) => {
    ...
    console.log('produsele in fctie de categorie', selectedProducts)

    let productsComponents = selectedProducts.map(prodObject => <Product getEditData={getEditData} products={selectedProducts} onClick={handleCheckbox} product={prodObject} key={prodObject.id} />);
    console.log('verificam produslee mapate',selectedProducts)
    return (
